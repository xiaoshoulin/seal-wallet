...
import           Seal.Wallet.Kernel.Types
...
import           Seal.Chain.Ssc (defaultSscPayload)
import           Seal.Chain.Txp (TxAux (..), TxId, TxIn (..), TxOut (..),
                     TxOutAux (..), Utxo, txAttributes)
import           Seal.Chain.Update (HasUpdateConfiguration, updateConfiguration)
import           Seal.Client.Txp
import           Seal.Core
import           Seal.Core.Chrono
import           Seal.Crypto
import           Seal.DB.Block (RawPayload (..), createMainBlockPure)
import           Serokell.Util (listJson)
import           Test.Seal.Chain.Genesis.Dummy (dummyConfig, dummyEpochSlots,
                     dummyK)
...
instance DSL.Hash h Addr => Interpret DSL2Cardano h (DSL.Transaction h Addr) where
  ...
  int t = do
      let currentTime = getSomeTimestamp -- Seal.Core.Timestamp $ minBound
