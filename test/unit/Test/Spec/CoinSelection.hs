...
import           Seal.Binary.Class (Bi (encode), toLazyByteString)
import           Seal.Chain.Genesis as Genesis (Config (..))
import qualified Seal.Chain.Txp as Core
import           Seal.Core (Coeff (..), TxSizeLinear (..), unsafeIntegerToCoin)
import qualified Seal.Core as Core
import           Seal.Core.Attributes (mkAttributes)
import           Seal.Crypto (ProtocolMagic (..), SecretKey)
import           Serokell.Data.Memory.Units (Byte, fromBytes)
import           Serokell.Util.Text (listJsonIndent)

import           Util.Buildable

import           Seal.Wallet.Kernel.CoinSelection (CoinSelFinalResult (..),
                     CoinSelHardErr (..), CoinSelPolicy,
                     CoinSelectionOptions (..), ExpenseRegulation (..),
                     InputGrouping (..), estimateMaxTxInputsExplicitBounds,
                     largestFirst, newOptions, random)
import           Seal.Wallet.Kernel.CoinSelection.FromGeneric (Cardano,
                     estimateCardanoFee, estimateHardMaxTxInputsExplicitBounds)
import           Seal.Wallet.Kernel.Transactions (mkStdTx)
import           Seal.Wallet.Kernel.Util.Core (paymentAmount, utxoBalance,
                     utxoRestrictToInputs)
import           Seal.Crypto.Signing.Safe (fakeSigner)
import           Test.Seal.Configuration (withProvidedMagicConfig)
