...
import           Seal.Wallet.API.V1.Types (Account (accAddresses),
                     WalletAddress (..))
import           Seal.Wallet.Client.Http (BatchImportResult, ClientError)
...
spec = do
    ...
    scenario "ADDRESSES_IMPORT_06 - Returns error when address is invalid" $ do
        ...
        verify (response :: Either ClientError (BatchImportResult Text))
            [ expectJSONError "Not a valid Seal Address"
            ]
