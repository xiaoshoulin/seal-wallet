...
import           Seal.Wallet.Client.Http (ClientError, Wallet)
    ...
    forM_ [CreateWallet, RestoreWallet] $ \operation -> describe (show operation) $ do
        ...
        describe "WALLETS_CREATE_09 - One cannot create/restore wallet with assurance level other than 'strict' or 'normal'" $ do
            let matrix =
                    [ ( "Arbitrary String"
                      , russianWalletName
                      , [ expectJSONError "Error in $.assuranceLevel: When parsing Seal.Wallet.API.V1.Types.AssuranceLevel expected a String with the tag of a constructor but got" ]
                      )
                    , ( "Empty String"
                      , ""
                      , [ expectJSONError "Error in $.assuranceLevel: When parsing Seal.Wallet.API.V1.Types.AssuranceLevel expected a String with the tag of a constructor but got" ]
        ...
        describe "WALLETS_CREATE_10 - One cannot create/restore without all required parameters (operation)" $ do
            let matrix =
                    [ ( "Missing operation"
                      ...
                      , [ expectJSONError "When parsing the record newWallet of type Seal.Wallet.API.V1.Types.NewWallet the key operation was not present." ]
                      )
                    , ( "Missing backupPhrase"
                      ...
                      , [ expectJSONError "When parsing the record newWallet of type Seal.Wallet.API.V1.Types.NewWallet the key backupPhrase was not present." ]
                      )
                    , ( "Missing assuranceLevel"
                      ...
                      , [ expectJSONError "When parsing the record newWallet of type Seal.Wallet.API.V1.Types.NewWallet the key assuranceLevel was not present." ]
                      )
                    , ( "Missing name"
                      , [json|{
                            "operation": #{operation},
                      ...
                      , [ expectJSONError "When parsing the record newWallet of type Seal.Wallet.API.V1.Types.NewWallet the key name was not present." ]
                      )
                    ]
    ...
    describe "WALLETS_CREATE_12 - Cannot perform operation other than 'create' or 'restore'" $ do
        let matrix =
                [ ( "Invalid operation name"
                  ...
                  , [ expectJSONError "Error in $.operation: When parsing Seal.Wallet.API.V1.Types.WalletOperation expected a String with the tag of a constructor but got" ]
                  )
