...
-}
module Seal.Wallet.Server.CLI where
...
getWalletNodeOptions = execParser programInfo
  where
    programInfo = info (helper <*> versionOption <*> walletStartupOptionsParser) $
        fullDesc <> progDesc "Seal edge node w/ wallet."
                 <> header "Seal edge node."
...
walletBackendParamsParser = mkWalletBackendParams
  ...
  where
    ...
    mkWalletBackendParams auth caCrt srvCrt srvKey clCrt clKey disabled =
        WalletBackendParams
