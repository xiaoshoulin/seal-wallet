...
module Seal.Wallet.API.V1.Types (
  ...
  , PaymentSource (..)
  , PaymentDistribution (..)
  , GoldPaymentDistribution (..)
  , DollarPaymentDistribution (..)
  , AccountInputDistribution (..)
  , AccountOutputDistribution (..)
  , Transaction (..)
  , TransactionType (..)
...
--   , mkBlockchainHeight
  ...
  , disToAccountOut
  , toCmd
  , module Seal.Wallet.Types.UtxoStatistics
  ) where
...
import           Seal.Wallet.API.Response.JSend (HasDiagnostic (..),
                     noDiagnosticKey)
import           Seal.Wallet.API.Types.UnitOfMeasure (MeasuredIn (..),
                     UnitOfMeasure (..))
import           Seal.Wallet.API.V1.Errors (ToHttpErrorStatus (..),
                     ToServantError (..))
import           Seal.Wallet.API.V1.Generic (jsendErrorGenericParseJSON,
                     jsendErrorGenericToJSON)
import           Seal.Wallet.API.V1.Info
import           Seal.Wallet.API.V1.Swagger.Example (Example, example)
import           Seal.Wallet.Chain.Arbitrary ()
import           Seal.Wallet.Core (coinToInteger)
import           Seal.Wallet.Kernel.AddressPoolGap (AddressPoolGap)
import           Seal.Wallet.Types.UtxoStatistics
...
import           Seal.Chain.Update (ApplicationName (..), SoftwareVersion (..))
import qualified Seal.Client.Txp.Util as Core
import           Seal.Crypto (PublicKey (..), decodeHash, hashHexF)
import qualified Seal.Crypto.Signing as Core
import           Seal.Infra.Communication.Types.Protocol ()
import           Seal.Infra.Diffusion.Subscription.Status
                     (SubscriptionStatus (..))
import           Seal.Infra.Util.LogSafe (BuildableSafeGen (..), buildSafe,
...
instance ToJSON WalletCoin where
    toJSON (WalletCoin c) = toJSON . unsafeGetCoin $ c


unsafeGetCoin :: Core.Coin -> Word64
unsafeGetCoin = Core.getCoin
...
newtype WalGoldCoin = WalGoldCoin { unWalGoldCoin :: Core.GoldCoin }
    deriving (Eq, Generic, Ord, Show)

deriveSafeBuildable ''WalGoldCoin
instance BuildableSafeGen WalGoldCoin where
    buildSafeGen sl (WalGoldCoin c) =
        bprint (plainOrSecureF sl build (fconst "<hidden amount>")) c

instance ToJSON WalGoldCoin where
    toJSON (WalGoldCoin c) = toJSON . unsafeGetGoldCoin $ c


unsafeGetGoldCoin :: Core.GoldCoin -> Word64
unsafeGetGoldCoin = Core.getGoldCoin


instance FromJSON WalGoldCoin where
    parseJSON v = WalGoldCoin <$> parseJSON v

instance Arbitrary WalGoldCoin where
    arbitrary = fmap WalGoldCoin arbitrary

instance ToSchema WalGoldCoin where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "WalGoldCoin") $ mempty
            & type_ .~ SwaggerNumber
            -- & maximum_ .~ Just (fromIntegral Core.maxCoinVal)

newtype WalGoldDollar = WalGoldDollar { unWalGoldDollar :: Core.GoldDollar }
    deriving (Eq, Generic, Ord, Show)

deriveSafeBuildable ''WalGoldDollar
instance BuildableSafeGen WalGoldDollar where
    buildSafeGen sl (WalGoldDollar c) =
        bprint (plainOrSecureF sl build (fconst "<hidden amount>")) c

instance ToJSON WalGoldDollar where
    toJSON (WalGoldDollar c) = toJSON . unsafeGetGoldDollar $ c


unsafeGetGoldDollar :: Core.GoldDollar -> Word64
unsafeGetGoldDollar = Core.getGoldDollar


instance FromJSON WalGoldDollar where
    parseJSON v = WalGoldDollar <$> parseJSON v

instance Arbitrary WalGoldDollar where
    arbitrary = fmap WalGoldDollar arbitrary

instance ToSchema WalGoldDollar where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "WalGoldDollar") $ mempty
            & type_ .~ SwaggerNumber
            -- & maximum_ .~ Just (fromIntegral Core.maxCoinVal)
...
newtype WalSlotId = WalSlotId { unWalSlotId :: Maybe Core.SlotId }
    deriving (Eq, Generic, Ord, Show)

deriveSafeBuildable ''WalSlotId
instance BuildableSafeGen WalSlotId where
    buildSafeGen _ (WalSlotId slot) = bprint build slot

instance Buildable [WalSlotId] where
    build = bprint listJson

instance ToJSON WalSlotId where
    toJSON (WalSlotId slot) = toJSON slot

instance FromJSON WalSlotId where
    parseJSON v = WalSlotId <$> parseJSON v

instance Arbitrary WalSlotId where
    arbitrary = fmap WalSlotId arbitrary

instance ToSchema WalSlotId where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "Address") $ mempty
            & type_ .~ SwaggerString
            & format ?~ "base58"

instance ToHttpApiData WalSlotId where
    toQueryParam (WalSlotId s) = sformat build s

instance ToHttpApiData Core.EpochIndex where
    toQueryParam (Core.EpochIndex e) = sformat build e

deriving instance Hashable WalSlotId
deriving instance NFData WalSlotId
...
newtype WalCoinGroup = WalCoinGroup { unWalCoinGroup :: Core.CoinGroup }
    deriving (Eq, Generic, Ord, Show)

deriveSafeBuildable ''WalCoinGroup
instance BuildableSafeGen WalCoinGroup where
    buildSafeGen _ (WalCoinGroup coingroup) =
        bprint ("coingroup: " % buildTrunc build) coingroup

instance Buildable [WalCoinGroup] where
    build = bprint listJson

instance ToJSON WalCoinGroup where
    toJSON (WalCoinGroup Core.CoinGroup{..}) = object [
        "seal" .= Core.moneyToInteger cgSeal,
        "gold" .= Core.moneyToInteger cgGold,
        "dollar" .= Core.moneyToInteger cgDollar ]

instance FromJSON WalCoinGroup where
    parseJSON c = WalCoinGroup <$> parseJSON c

instance Arbitrary WalCoinGroup where
    arbitrary = fmap WalCoinGroup arbitrary

instance ToSchema WalCoinGroup where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "WalCoinGroup") $ mempty
            & type_ .~ SwaggerNumber
...
deriving instance Hashable WalCoinGroup
deriving instance NFData WalCoinGroup

newtype WalCmd = WalCmd { unWalCmd :: Core.Cmd }
    deriving (Eq, Generic, Ord, Show)

deriveSafeBuildable ''WalCmd
instance BuildableSafeGen WalCmd where
    buildSafeGen _ (WalCmd cmd) =
        bprint ("cmd: " % buildTrunc build) cmd

instance Buildable [WalCmd] where
    build = bprint listJson

instance ToJSON WalCmd where
    toJSON (WalCmd c) = toJSON c

instance FromJSON WalCmd where
    parseJSON c = WalCmd <$> parseJSON c

instance Arbitrary WalCmd where
    arbitrary = fmap WalCmd arbitrary

instance ToSchema WalCmd where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "Cmd") $ mempty
            & type_ .~ SwaggerString
            & format ?~ "base58"

toCmd :: WalCmd -> Core.Cmd
toCmd = unWalCmd
--
...
data Wallet = Wallet {
    ...
    , walGoldBalance                :: !WalGoldCoin
    , walDollarBalance              :: !WalGoldDollar
...
instance ToSchema Wallet where
    declareNamedSchema =
        genericSchemaDroppingPrefix "wal" (\(--^) props -> props
            ...
            & "goldBalance"
            --^ "Current gold balance, in Lovelace."
            & "dollarBalance"
            --^ "Current dollar balance, in Lovelace."
            & "hasSpendingPassword"
...
instance BuildableSafeGen Wallet where
  buildSafeGen sl Wallet{..} = bprint
    ...
    % "\n  gold balance: " % buildSafe sl
    % "\n  dollar balance: " % buildSafe sl
    % "\n  created at: " % build
    % "\n  " % build
...
---------------- seal start -------------------
data GoldPaymentDistribution = GoldPaymentDistribution {
      gpdAddress :: !WalAddress
    , gpdAmount  :: !WalGoldCoin
    } deriving (Show, Ord, Eq, Generic)

deriveJSON Aeson.defaultOptions ''GoldPaymentDistribution

instance ToSchema GoldPaymentDistribution where
  declareNamedSchema =
    genericSchemaDroppingPrefix "gpd" (\(--^) props -> props
      & ("address" --^ "Address to map coins to.")
      & ("amount"  --^ "Amount of gold to bind, in Lovelace.")
    )

instance Arbitrary GoldPaymentDistribution where
  arbitrary = GoldPaymentDistribution <$> arbitrary
                                  <*> arbitrary

deriveSafeBuildable ''GoldPaymentDistribution
instance BuildableSafeGen GoldPaymentDistribution where
    buildSafeGen sl GoldPaymentDistribution{..} = bprint ("{"
        %" address="%buildSafe sl
        %" amount="%buildSafe sl
        %" }")
        gpdAddress
...
data DollarPaymentDistribution = DollarPaymentDistribution {
      dpdAddress :: !WalAddress
    , dpdAmount  :: !WalGoldDollar
    } deriving (Show, Ord, Eq, Generic)

deriveJSON Aeson.defaultOptions ''DollarPaymentDistribution

instance ToSchema DollarPaymentDistribution where
  declareNamedSchema =
    genericSchemaDroppingPrefix "dpd" (\(--^) props -> props
      & ("address" --^ "Address to map coins to.")
      & ("amount"  --^ "Amount of dollar to bind, in Lovelace.")
    )

instance Arbitrary DollarPaymentDistribution where
  arbitrary = DollarPaymentDistribution <$> arbitrary
                                  <*> arbitrary

deriveSafeBuildable ''DollarPaymentDistribution
instance BuildableSafeGen DollarPaymentDistribution where
    buildSafeGen sl DollarPaymentDistribution{..} = bprint ("{"
        %" address="%buildSafe sl
        %" amount="%buildSafe sl
        %" }")
        dpdAddress
...
newtype WalNonce = WalNonce { unWalNonce :: Txp.Nonce }
    deriving (Eq, Generic, Ord, Show)

deriveSafeBuildable ''WalNonce
instance BuildableSafeGen WalNonce where
    buildSafeGen _ (WalNonce nonce) = bprint build nonce

instance Arbitrary WalNonce where
    arbitrary = fmap WalNonce arbitrary

deriving instance FromJSON WalNonce
deriving instance ToJSON WalNonce

instance ToSchema WalNonce where
    declareNamedSchema _ = declareNamedSchema (Proxy @Text)

-- instance Buildable (SecureLog WalNonce) where
--     build _ = "<nonce>"

data AccountInputDistribution = AccountInputDistribution {
      aipdAddress :: !WalAddress
    , aipdNonce   :: !WalNonce
    , aipdAmount  :: !WalCoinGroup
    } deriving (Show, Ord, Eq, Generic)

deriveJSON Aeson.defaultOptions ''AccountInputDistribution

instance ToSchema AccountInputDistribution where
    declareNamedSchema =
        genericSchemaDroppingPrefix "apd" (\(--^) props -> props
        & ("address" --^ "Address to map coingroups to.")
        & ("nonce"   --^ "Account transaction record.")
        & ("amount"  --^ "Amount of coin group to bind, in Lovelace.")
        )

instance Arbitrary AccountInputDistribution where
    arbitrary = AccountInputDistribution <$> arbitrary
                                                <*> arbitrary
                                                <*> arbitrary

deriveSafeBuildable ''AccountInputDistribution

instance BuildableSafeGen AccountInputDistribution where
    buildSafeGen sl AccountInputDistribution{..} = bprint ("{"
        %" address="%buildSafe sl
        %" nonce="%buildSafe sl
        %" amount="%buildSafe sl
        %" }")
...
data AccountOutputDistribution = AccountOutputDistribution {
      apdAddress :: !WalAddress
    , apdSeal    :: !WalletCoin
    , apdGold    :: !WalGoldCoin
    , apdDollar  :: !WalGoldDollar
    } deriving (Show, Ord, Eq, Generic)

deriveJSON Aeson.defaultOptions ''AccountOutputDistribution

instance ToSchema AccountOutputDistribution where
    declareNamedSchema =
        genericSchemaDroppingPrefix "apd" (\(--^) props -> props
        & ("address" --^ "Address to map coingroups to.")
        & ("seal"    --^ "Amount of coin to bind, in Lovelace.")
        & ("gold"    --^ "Amount of gold coin to bind, in Lovelace.")
        & ("dollar"  --^ "Amount of gold dollar to bind, in Lovelace.")
        )

instance Arbitrary AccountOutputDistribution where
    arbitrary = AccountOutputDistribution <$> arbitrary
                                          <*> arbitrary
                                          <*> arbitrary
                                          <*> arbitrary

deriveSafeBuildable ''AccountOutputDistribution

instance BuildableSafeGen AccountOutputDistribution where
    buildSafeGen sl AccountOutputDistribution{..} = bprint ("{"
        %" address="%buildSafe sl
        %" seal="%buildSafe sl
        %" gold="%buildSafe sl
        %" dollar="%buildSafe sl
...
disToAccountOut :: AccountOutputDistribution -> Txp.AccountOut
disToAccountOut AccountOutputDistribution{..} = 
     let doAccount = Core.Account $ unWalAddress apdAddress
         doValue = Core.CoinGroup (unWalletCoin apdSeal) 
                                  (unWalGoldCoin apdGold) 
                                  (unWalGoldDollar apdDollar)
     in Txp.DepositOutput{..}
...
data Payment = Payment
  { pmtRemark              :: !(Maybe Text)
  , pmtSource              :: !PaymentSource
  , pmtDestinations        :: ![PaymentDistribution]
  , pmtDestinationgolds    :: ![GoldPaymentDistribution]
  , pmtDestinationdollars  :: ![DollarPaymentDistribution]
  , pmtDestinationaccounts :: ![AccountOutputDistribution]
  , pmtCmds                :: ![WalCmd]
...
instance ToSchema Payment where
  declareNamedSchema =
    genericSchemaDroppingPrefix "pmt" (\(--^) props -> props
      & ("remark"              --^ "remark for the payment.")
      & ("source"              --^ "Source for the payment.")
      & ("destinations"        --^ "One or more destinations for the payment.")
      & ("destinationgolds"    --^ "One or more destinationgolds for the payment.")
      & ("destinationdollars"  --^ "One or more destinationdollars for the payment.")
      & ("destinationaccounts" --^ "One or more destinationaccounts for the payment.")
      & ("cmds"                --^ "One or more cmds for the payment.")
...
instance BuildableSafeGen Payment where
    buildSafeGen sl (Payment{..}) = bprint ("{"
        %" remark="%buildSafeMaybe mempty sl
        %" source="%buildSafe sl
        %" destinations="%buildSafeList sl
        %" destinationgolds="%buildSafeList sl
        %" destinationdollars="%buildSafeList sl
        %" destinationaccounts="%buildSafeList sl
        %" cmds="%buildSafeList sl
        ...
        (toList pmtDestinationgolds)
        (toList pmtDestinationdollars)
        (toList pmtDestinationaccounts)
        (toList pmtCmds)
        pmtGroupingPolicy
        pmtSpendingPassword
...
data Transaction = Transaction
  ...
  , txAmount         :: !WalCoinGroup
  , txInputs         :: ![PaymentDistribution]
  , txOutputs        :: ![PaymentDistribution]
  , txGoldInputs     :: ![GoldPaymentDistribution]
  , txGoldOutputs    :: ![GoldPaymentDistribution]
  , txDollarInputs   :: ![DollarPaymentDistribution]
  , txDollarOutputs  :: ![DollarPaymentDistribution]
  , txAccountInputs  :: ![AccountInputDistribution]
  , txAccountOutputs :: ![AccountOutputDistribution]
  , txCmds           :: ![WalCmd]
...
instance ToSchema Transaction where
  declareNamedSchema =
    genericSchemaDroppingPrefix "tx" (\(--^) props -> props
      ...
      & ("amount"         --^ "CoinGroups moved as part of the transaction, in Lovelace.")
      & ("inputs"         --^ "One or more input money distributions.")
      & ("outputs"        --^ "One or more ouputs money distributions.")
      & ("goldinputs"     --^ "One or more gold input money distributions.")
      & ("goldoutputs"    --^ "One or more gold ouputs money distributions.")
      & ("dollarinputs"   --^ "One or more dollar inputs money distributions.")
      & ("dollaroutputs"  --^ "One or more dollar ouputs money distributions.")
      & ("accountinputs"  --^ "One or more account inputs money distributions.")
      & ("accountoutputs" --^ "One or more account ouputs money distributions.")
      & ("cmds"           --^ "One or more tokens distributions.")
...
instance BuildableSafeGen Transaction where
    buildSafeGen sl Transaction{..} = bprint ("{"
        ...
        %" goldinputs="%buildSafeList sl
        %" goldoutputs="%buildSafeList sl
        %" dollarinputs="%buildSafeList sl
        %" dollaroutputs="%buildSafeList sl
        %" accountinputs="%buildSafeList sl
        %" accountoutputs="%buildSafeList sl
        %" cmds="%buildSafeList sl
        %" type="%buildSafe sl
        ...
        (toList txOutputs)
        (toList txGoldInputs)
        (toList txGoldOutputs)
        (toList txDollarInputs)
        (toList txDollarOutputs)
        (toList txAccountInputs)
        (toList txAccountOutputs)
        (toList txCmds)
        txType
        txDirection
...
instance Example GoldPaymentDistribution
instance Example DollarPaymentDistribution
instance Example AccountInputDistribution
instance Example AccountOutputDistribution
...
instance Example WalGoldCoin
instance Example WalGoldDollar
instance Example WalCmd
instance Example WalCoinGroup
...
instance FromJSON CConfirmedProposalState where
    parseJSON = genericParseJSON Aeson.defaultOptions