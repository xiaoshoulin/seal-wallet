...
module Seal.Wallet.API.V1.ReifyWalletError (
...
import           Seal.Core (decodeTextAddress)
...
import           Seal.Wallet.Kernel.CoinSelection.Generic
                     (CoinSelHardErr (..))
import           Seal.Wallet.Kernel.DB.AcidState (NewPendingError (..))
...
import           Seal.Wallet.WalletLayer
import           Seal.Wallet.WalletLayer.Kernel.Conv (toRootId)
...
noHdAddressForSrcAddress e = case e of
    ...
    (HD.UnknownHdSealAddress _coreAddr) ->
        V1.AddressNotFound
