...
module Seal.Wallet.API.V1.Swagger where

import           Universum hiding (get, put)

import           Seal.Wallet.API.Indices (ParamNames)
...
import           Seal.Wallet.API.V1.Generic (gconsName)
import           Seal.Wallet.API.V1.Parameters
import           Seal.Wallet.API.V1.Swagger.Example
import           Seal.Wallet.API.V1.Types
import           Seal.Wallet.TypeLits (KnownSymbols (..))

import           Seal.Chain.Update (SoftwareVersion (svNumber))
import           Seal.Util.CompileInfo (CompileTimeInfo, ctiGitRevision)
...
import qualified Seal.Crypto.Hashing as Crypto
...
|] where
  ...
  sampleAddress = WalAddress Core.Address
      ...
      , Core.addrAttributes =
          Core.mkAttributes $ Core.AddrAttributes Nothing Core.BootstrapEraDistr Nothing
      , Core.addrType =
          Core.ATPubKey
      }
