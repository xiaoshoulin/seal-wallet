...
module Seal.Wallet.API.V1.Info where
...
import           Seal.Infra.Diffusion.Subscription.Status
                     (SubscriptionStatus (..))
import           Seal.Infra.Util.LogSafe (BuildableSafeGen (..), SecureLog (..),
                     deriveSafeBuildable)
import           Seal.Node.NodeStateAdaptor (SecurityParameter (..))
import           Seal.Wallet.API.Util.Example
import           Seal.Wallet.API.Util.Servant (APIResponse, CustomQueryFlag, Flaggable (..),
                     HasCustomQueryFlagDescription (..), ValidJSON)
...
instance BuildableSafeGen SubscriptionStatus where
    buildSafeGen _ Subscribed  = "Subscribed"
    buildSafeGen _ Subscribing = "Subscribing"
...
instance Arbitrary Core.SoftwareVersion where
    arbitrary = genericArbitrary
    shrink = genericShrink

instance Arbitrary Core.ApplicationName where
    arbitrary =
        Core.ApplicationName .
        toText . map selectAlpha . take Core.applicationNameMaxLength <$>
        arbitrary
      where
