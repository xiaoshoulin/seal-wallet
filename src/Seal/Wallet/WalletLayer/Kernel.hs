...
module Seal.Wallet.WalletLayer.Kernel
...
import           Seal.Chain.Block (Blund, blockHeader, headerHash, prevBlockL)
...
import           Seal.Util.Wlog (Severity (Debug, Warning))

import qualified Seal.Wallet.Kernel as Kernel
import qualified Seal.Wallet.Kernel.Actions as Actions
import qualified Seal.Wallet.Kernel.BListener as Kernel
import           Seal.Wallet.Kernel.DB.AcidState (dbHdWallets)
import           Seal.Wallet.Kernel.DB.HdWallet (hdAccountRestorationState,
                     hdRootId, hdWalletsRoots)
import qualified Seal.Wallet.Kernel.DB.Read as Kernel
import qualified Seal.Wallet.Kernel.DB.Util.IxSet as IxSet
import           Seal.Wallet.Kernel.Diffusion (WalletDiffusion (..))
import           Seal.Wallet.Kernel.Keystore (Keystore)
import           Seal.Wallet.Kernel.NodeStateAdaptor
import qualified Seal.Wallet.Kernel.Read as Kernel
import qualified Seal.Wallet.Kernel.Restore as Kernel
import           Seal.Wallet.WalletLayer (ActiveWalletLayer (..),
                     PassiveWalletLayer (..))
import qualified Seal.Wallet.WalletLayer.Kernel.Accounts as Accounts
import qualified Seal.Wallet.WalletLayer.Kernel.Active as Active
import qualified Seal.Wallet.WalletLayer.Kernel.Addresses as Addresses
import qualified Seal.Wallet.WalletLayer.Kernel.Internal as Internal
import qualified Seal.Wallet.WalletLayer.Kernel.Transactions as Transactions
import qualified Seal.Wallet.WalletLayer.Kernel.Wallets as Wallets
