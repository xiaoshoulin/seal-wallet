module Seal.Wallet.WalletLayer.Kernel.Accounts (
    createAccount
  , getAccount
  , getAccountBalance
  , getAccountAddresses
...
import           Seal.Core (CoinGroup (..))
import           Seal.Crypto.Signing

import           Seal.Wallet.API.Request (RequestParams, SortOperations (..))
import           Seal.Wallet.API.Request.Filter (FilterOperations (..))
import           Seal.Wallet.API.Response (APIResponse, respondWith)
import           Seal.Wallet.API.V1.Types (WalletAddress)
...
import           Seal.Wallet.Kernel.DB.Read (addressesByAccountId)
...
import           Seal.Wallet.WalletLayer (CreateAccountError (..),
...
getAccountBalance wId accIx snapshot = runExcept $ do
    ...
    withExceptT GetAccountError $ exceptT $ do
        coin       <- Kernel.currentTotalBalance snapshot accId
        goldcoin   <- Kernel.currentTotalGoldBalance snapshot accId
        golddollar <- Kernel.currentTotalDollarBalance snapshot accId
        Right $ V1.AccountBalance $ V1.WalCoinGroup (CoinGroup coin goldcoin golddollar)
