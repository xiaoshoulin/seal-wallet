module Seal.Wallet.WalletLayer.Kernel.Internal (
    resetWalletState
  , importWallet
  ) where
...
import           Seal.Wallet.Kernel.DB.AcidState (ClearDB (..))
...
import           Seal.Wallet.WalletLayer (CreateWallet (..),
                     ImportWalletError (..))
import           Seal.Wallet.WalletLayer.Kernel.Wallets (createWallet)
