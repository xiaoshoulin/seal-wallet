...
module Seal.Wallet.WalletLayer.Kernel.Conv (
...
import           Seal.Client.Txp.Util (InputSelectionPolicy (..))
import           Seal.Core (Address, BlockCount (..), CoinGroup (..))
import           Seal.Crypto (AesKey, RedeemSecretKey, aesDecrypt,
                     redeemDeterministicKeyGen)

import           Seal.Mnemonic (mnemonicToAesKey)
...
import           Seal.Wallet.Kernel.CoinSelection.FromGeneric
                     (InputGrouping (..))
import           Seal.Wallet.Kernel.DB.BlockMeta (addressMetaIsUsed)
...
toAccount snapshot account = V1.Account {
    ...
    , accAmount    = V1.WalCoinGroup (CoinGroup accountAvailableBalance 
                                                accountAvailableGoldBalance
                                                accountAvailableDollarBalance)
  ...
  where
    ...
    accountAvailableGoldBalance = account ^. HD.hdAccountState . HD.hdAccountStateCurrent (to cpAvailableGoldBalance)
    accountAvailableDollarBalance = account ^. HD.hdAccountState . HD.hdAccountStateCurrent (to cpAvailableDollarBalance)
...
toWallet db hdRoot = V1.Wallet {
    ...
    , walGoldBalance                = V1.WalGoldCoin (Kernel.rootTotalGoldBalance db rootId)
    , walDollarBalance              = V1.WalGoldDollar (Kernel.rootTotalDollarBalance db rootId)
...
toSealAddress :: HD.HdAddress -> Address
toSealAddress hdAddr = hdAddr ^. HD.hdAddressAddress . fromDb
