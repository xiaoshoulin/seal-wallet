...
module Seal.Wallet.WalletLayer.Kernel.Wallets (
      createWallet
    , createEosWallet
    , updateWallet
    , updateEosWallet
...
import           Seal.Wallet.Kernel.Internal (walletKeystore, walletMeta,
...
import           Seal.Wallet.Kernel.Read (WalletOwnership (..),
...
import           Seal.Wallet.Kernel.Restore (blundToResolvedBlock,
                     restoreWallet)
import qualified Seal.Wallet.Kernel.Wallets as Kernel
import           Seal.Wallet.WalletLayer (CreateWallet (..),
  ...
  where
    ...
    restoreFromESK nm esk pwd walletName hdAssuranceLevel = runExceptT $ do
        ...
        case mbHdAddress of
            ...
            Just hdAddress -> do
                (root, coins, goldCoins, dollarCoins) <- withExceptT (CreateWalletError . Kernel.CreateWalletFailed) $ ExceptT $
                    restoreWallet
                      wallet
                      (pwd /= emptyPassphrase)
                ...
                updateSyncState wallet 
                                rootId 
                                (root' { 
                                            V1.walBalance       = V1.WalletCoin coins,
                                            V1.walGoldBalance   = V1.WalGoldCoin goldCoins,
                                            V1.walDollarBalance = V1.WalGoldDollar dollarCoins
                                        }
                                )
    ...
    mkRoot v1WalletName v1AssuranceLevel hdRoot = V1.Wallet {
        ...
        , walGoldBalance                = V1.WalGoldCoin (mkGoldCoin 0)
        , walDollarBalance              = V1.WalGoldDollar (mkGoldDollar 0)
