...
module Seal.Wallet.Kernel.Submission (
    -- * Public API
      newWalletSubmission
...
import           Control.Lens (Getter, anon, at)
...
import           Seal.Chain.Txp (topsortTxs)
import           Seal.Crypto.Hashing (WithHash (..))
...
partitionSendable pendingSets xs =
    ...
    where
        ...
        dependsOnFutureTx canSendIds (ScheduleSend accId _ txAux _) =
            let inputs = List.foldl' updateFn mempty $ Txp._txUtxoInputs (Txp.taTx txAux)
