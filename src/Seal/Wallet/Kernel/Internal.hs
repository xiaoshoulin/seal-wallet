...
-- Seal specific types (except those types that appear in the translation
-- of the UTxO DSL), so that we can test it outside of a node context also
-- (in unit tests).
module Seal.Wallet.Kernel.Internal (
    -- * Passive wallet
    PassiveWallet(..)
    -- ** Lenses
  , walletKeystore
...
import           Seal.Core (BlockCount, FlatSlotId)
