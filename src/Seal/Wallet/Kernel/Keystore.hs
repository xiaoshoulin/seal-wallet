...
module Seal.Wallet.Kernel.Keystore (
      Keystore -- opaque
    , DeletePolicy(..)
    , ReplaceResult(..)
...
import           Seal.Crypto (EncryptedSecretKey, hash)
import           Seal.Util.UserSecret (UserSecret, getUSPath, isEmptyUserSecret,
                     readUserSecret, takeUserSecret, usKeys, usWallet,
                     writeRaw, writeUserSecretRelease, _wusRootKey)
import           Seal.Util.Wlog (addLoggerName)
