...
module Seal.Wallet.Kernel.Ed25519Bip44
    ( ChangeChain(..)
    , purposeIndex
    , coinTypeIndex
...
import           Seal.Crypto.Wallet (DerivationScheme (DerivationScheme2),
                     deriveXPrv, deriveXPub, generate)
import           Seal.Mnemonic (Mnemonic, entropyToByteString,
                     mnemonicToEntropy)
import           Seal.Crypto (EncryptedSecretKey (..), PassPhrase,
...
--
-- Coin type is a constant, set for each cryptocoin. For Seal this constant is set
...
-- using derivation scheme 2 (please see 'Seal-crypto' package).
deriveAddressPublicKey
