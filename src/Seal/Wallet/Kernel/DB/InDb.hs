...
module Seal.Wallet.Kernel.DB.InDb
    ( InDb(..)
    , fromDb
    ) where
...
instance SC.SafeCopy (InDb Core.Account) where
    getCopy = SC.contain $ do
        InDb (a :: Core.Address) <- SC.safeGet
        pure (InDb (Core.Account a))
    putCopy (InDb (Core.Account a)) = SC.contain $ do
        SC.safePut (InDb (a :: Core.Address))

instance SC.SafeCopy (InDb Core.CoinGroup) where
    getCopy = SC.contain $ do
        InDb (c :: Core.Coin) <- SC.safeGet
        InDb (g :: Core.GoldCoin) <- SC.safeGet
        InDb (d :: Core.GoldDollar) <- SC.safeGet
        pure (InDb (Core.CoinGroup c g d))
    putCopy (InDb (Core.CoinGroup c g d)) = SC.contain $ do
        SC.safePut (InDb (c :: Core.Coin))
        SC.safePut (InDb (g :: Core.GoldCoin))
        SC.safePut (InDb (d :: Core.GoldDollar))

instance SC.SafeCopy (InDb Txp.Nonce) where
    getCopy = SC.contain $ do
        n :: Word64 <- SC.safeGet
        pure (InDb n)
    putCopy (InDb n) = SC.contain $ do
        SC.safePut (n :: Word64)
...
instance SC.SafeCopy (InDb Core.AddrAttributes) where
    getCopy = SC.contain $ do
        ...
        yiac :: Maybe (InDb Core.AddrCategory) <- SC.safeGet
        pure (InDb (Core.AddrAttributes (fmap _fromDb yiap) ast (fmap _fromDb yiac)))
    putCopy (InDb (Core.AddrAttributes yap asr yac)) = SC.contain $ do
        SC.safePut (fmap InDb yap)
        SC.safePut (InDb asr)
        SC.safePut (fmap InDb yac)
...
instance SC.SafeCopy (InDb Core.AddrCategory) where
    getCopy = SC.contain $ fmap InDb $ do
        SC.safeGet >>= \case
            0 -> pure Core.ACMobile
            1 -> pure Core.ACUserAccount
            2 -> pure Core.ACContractAccount
            3 -> Core.ACUnknown <$> SC.safeGet
            (n :: Word8) -> fail
                $ "Expected one of 0,1,2,3 for tag of AddrStakeDistribution, got: "
                <> show n
    putCopy (InDb x) = SC.contain $ case x of
        Core.ACMobile          -> SC.safePut (0 :: Word8)
        Core.ACUserAccount     -> SC.safePut (1 :: Word8)
        Core.ACContractAccount -> SC.safePut (2 :: Word8)
        Core.ACUnknown a -> do
            SC.safePut (3 :: Word8)
            SC.safePut (a :: Word8)
...
instance SC.SafeCopy (InDb Core.GoldCoin) where
    getCopy = SC.contain $ do
       w :: Word64 <- SC.safeGet
       pure (InDb (Core.GoldCoin w))
    putCopy (InDb (Core.GoldCoin w)) = SC.contain $ do
       SC.safePut (w :: Word64)

instance SC.SafeCopy (InDb Core.GoldDollar) where
    getCopy = SC.contain $ do
       w :: Word64 <- SC.safeGet
       pure (InDb (Core.GoldDollar w))
    putCopy (InDb (Core.GoldDollar w)) = SC.contain $ do
       SC.safePut (w :: Word64)
...
instance SC.SafeCopy (InDb Txp.Tx) where
    getCopy = SC.contain $ do
        InDb (i :: [Txp.TxIn]) <- SC.safeGet
        InDb (o :: [Txp.TxOut]) <- SC.safeGet
        InDb (ai :: [Txp.AccountIn]) <- SC.safeGet
        InDb (ao :: [Txp.AccountOut]) <- SC.safeGet
        InDb (c :: [Core.Cmd]) <- SC.safeGet
        InDb (a :: Txp.TxAttributes) <- SC.safeGet
        pure (InDb (Txp.UnsafeTx i o ai ao c a))
    putCopy (InDb (Txp.UnsafeTx i o ai ao c a)) = SC.contain $ do
        SC.safePut (InDb i)
        SC.safePut (InDb o)
        SC.safePut (InDb ai)
        SC.safePut (InDb ao)
    ...
    getCopy = SC.contain $ do
        SC.safeGet >>= \case
            0 -> do
                InDb (a :: Core.Address) <- SC.safeGet
                InDb (c :: Core.Coin) <- SC.safeGet
                s :: Maybe (InDb Core.SlotId) <- SC.safeGet
                pure (InDb (Txp.TxOutSeal a c (fmap _fromDb s)))
            1 -> do
                InDb (a :: Core.Address) <- SC.safeGet
                InDb (c :: Core.GoldCoin) <- SC.safeGet
                pure (InDb (Txp.TxOutGold a c))
            2 -> do
                InDb (a :: Core.Address) <- SC.safeGet
                InDb (c :: Core.GoldDollar) <- SC.safeGet
                pure (InDb (Txp.TxOutDollar a c))
            3 -> do
                InDb (a :: Core.Address) <- SC.safeGet
                InDb (c :: Txp.IssueCert) <- SC.safeGet
                InDb (s :: Txp.IssueState) <- SC.safeGet
                pure (InDb (Txp.TxOutState a c s))
            (n :: Word8) -> fail
                $ "Expected 0,1,2,3 for tag of TxInWitness, got: "
                <> show n
    putCopy (InDb (Txp.TxOutSeal a c s)) = SC.contain $ do
        SC.safePut (0 :: Word8)
        SC.safePut (InDb a)
        SC.safePut (InDb c)
        SC.safePut (fmap InDb s)
    putCopy (InDb (Txp.TxOutGold a c)) = SC.contain $ do
        SC.safePut (1 :: Word8)
        SC.safePut (InDb a)
        SC.safePut (InDb c)
    putCopy (InDb (Txp.TxOutDollar a c)) = SC.contain $ do
        SC.safePut (2 :: Word8)
        SC.safePut (InDb a)
        SC.safePut (InDb c)
    putCopy (InDb (Txp.TxOutState a c s)) = SC.contain $ do
        SC.safePut (3 :: Word8)
        SC.safePut (InDb a)
        SC.safePut (InDb c)
        SC.safePut (InDb s)

instance SC.SafeCopy (InDb Txp.IssueCert) where
    getCopy = SC.contain $ do
        SC.safeGet >>= \case
            0 -> do
                InDb (g :: Core.GoldCoin) <- SC.safeGet
                (t :: Text) <- SC.safeGet
                pure (InDb (Txp.GoldCoinIssueCert g t))
            1 -> do
                InDb (d :: Core.GoldDollar) <- SC.safeGet
                InDb (g :: Core.GoldCoin) <- SC.safeGet
                (t :: Text) <- SC.safeGet
                pure (InDb (Txp.GoldDollarIssueCert d g t))
            2 -> do
                InDb (d :: Core.GoldDollar) <- SC.safeGet
                InDb (g :: Core.GoldCoin) <- SC.safeGet
                (t :: Text) <- SC.safeGet
                pure (InDb (Txp.GoldDollarDestroyCert d g t))
            (n :: Word8) -> fail
                $ "Expected 0,1,2 for tag of TxInWitness, got: "
                <> show n
    putCopy (InDb (Txp.GoldCoinIssueCert g t)) = SC.contain $ do
        SC.safePut (0 :: Word8)
        SC.safePut (InDb g)
        SC.safePut (t :: Text)
    putCopy (InDb (Txp.GoldDollarIssueCert d g t)) = SC.contain $ do
        SC.safePut (1 :: Word8)
        SC.safePut (InDb d)
        SC.safePut (InDb g)
        SC.safePut (t :: Text)
    putCopy (InDb (Txp.GoldDollarDestroyCert d g t)) = SC.contain $ do
        SC.safePut (2 :: Word8)
        SC.safePut (InDb d)
        SC.safePut (InDb g)
        SC.safePut (t :: Text)

instance SC.SafeCopy (InDb Txp.IssueState) where
    getCopy = SC.contain $ do
        SC.safeGet >>= \case
            0 -> do
                InDb (g :: Core.GoldCoin) <- SC.safeGet
                pure (InDb (Txp.GoldCoinState g))
            1 -> do
                InDb (d :: Core.GoldDollar) <- SC.safeGet
                InDb (g :: Core.GoldCoin) <- SC.safeGet
                pure (InDb (Txp.GoldDollarState d g))
            (n :: Word8) -> fail
                $ "Expected 0,1 for tag of TxInWitness, got: "
                <> show n
    putCopy (InDb (Txp.GoldCoinState g)) = SC.contain $ do
        SC.safePut (0 :: Word8)
        SC.safePut (InDb g)
    putCopy (InDb (Txp.GoldDollarState d g)) = SC.contain $ do
        SC.safePut (1 :: Word8)
        SC.safePut (InDb d)
        SC.safePut (InDb g)
...
instance SC.SafeCopy (InDb Core.AddrType) where
    getCopy = SC.contain $ fmap InDb $ do
        SC.safeGet >>= \case
            ...
            (n :: Word8) -> fail
                $ "Expected 0,2,3 for tag of AddrType, got: "
                <> show n
...
instance SC.SafeCopy (InDb Txp.SealAttributes) where
    getCopy = SC.contain $ do
        InDb (a :: Txp.Attachments) <- SC.safeGet
        pure (InDb (Txp.SealAttributes a))
    putCopy (InDb (Txp.SealAttributes a)) = SC.contain $ do
        SC.safePut (InDb a)

instance SC.SafeCopy (InDb Txp.AttachmentTag) where
    getCopy = SC.contain $ fmap InDb $ do
        SC.safeGet >>= \case
            0 -> pure Txp.Remarks
            1 -> Txp.Unknown <$> SC.safeGet
            (n :: Word8) -> fail
                $ "Expected 0,1 for tag of AddrType, got: "
                <> show n
    putCopy (InDb x) = SC.contain $ case x of
        Txp.Remarks -> SC.safePut (0 :: Word8)
        Txp.Unknown a -> do
            SC.safePut (1 :: Word8)
            SC.safePut (a :: Word8)
...
instance SC.SafeCopy (InDb ByteString) where
    getCopy = SC.contain $ do
        b :: ByteString <- SC.safeGet
        pure (InDb b)
    putCopy (InDb b) = SC.contain $ do
        SC.safePut (b :: ByteString)
...
instance SC.SafeCopy (InDb Txp.AccountIn) where
    getCopy = SC.contain $ do
        InDb (a :: Core.Account) <- SC.safeGet
        InDb (n :: Txp.Nonce) <- SC.safeGet
        InDb (c :: Core.CoinGroup) <- SC.safeGet
        pure (InDb (Txp.RedeemInput a n c))
    putCopy (InDb (Txp.RedeemInput a n c)) = SC.contain $ do
        SC.safePut (InDb a)
        SC.safePut (InDb n)
        SC.safePut (InDb c)

instance SC.SafeCopy (InDb Txp.AccountOut) where
    getCopy = SC.contain $ do
        InDb (a :: Core.Account) <- SC.safeGet
        InDb (c :: Core.CoinGroup) <- SC.safeGet
        pure (InDb (Txp.DepositOutput a c))
    putCopy (InDb (Txp.DepositOutput a c)) = SC.contain $ do
        SC.safePut (InDb a)
        SC.safePut (InDb c)

instance SC.SafeCopy (InDb Core.Cmd) where
    getCopy = SC.contain $ do
        c :: String <- SC.safeGet
        pure (InDb (Core.Cmd c))
    putCopy (InDb (Core.Cmd c)) = SC.contain $ do
        SC.safePut (c :: String)
        
instance (SC.SafeCopy (InDb a)) => SC.SafeCopy (InDb [a]) where
    getCopy = SC.contain $ do
        xsi :: [InDb a] <- SC.safeGet
        pure (InDb (fmap _fromDb xsi))
    putCopy (InDb sa) = SC.contain $ do
        let xsi :: [InDb a] = map InDb sa
        SC.safePut xsi
