...
module Seal.Wallet.Kernel.DB.AcidState (
    -- * Top-level database
    DB(..)
  , dbHdWallets
  , defDB
...
import           Seal.Chain.Txp (TxAux, TxId, Utxo)
...
import qualified Seal.Wallet.Kernel.DB.HdWallet.Delete as HD
import qualified Seal.Wallet.Kernel.DB.HdWallet.Update as HD
import           Seal.Wallet.Kernel.DB.InDb (InDb (InDb))
...
import           Seal.Wallet.Kernel.DB.Updates (Updates, noUpdates)
import qualified Seal.Wallet.Kernel.DB.Updates as Updates
...
import qualified Seal.Wallet.Kernel.DB.Util.Zoomable as Z
import           Seal.Wallet.Kernel.NodeStateAdaptor (SecurityParameter (..))
import           Seal.Wallet.Kernel.Prefiltering (PrefilteredBlock, pfbAddrs,
                     pfbByAccounts)
...
--  * V1 API defined in "Seal.Wallet.API.V1.*" (in @src/@)
data DB = DB {
...
-- 'createHdWallet'/'createWalletHdRnd' in "Seal.Wallet.Kernel.Wallets".
--
