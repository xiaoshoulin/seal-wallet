...
-- | HD wallets
module Seal.Wallet.Kernel.DB.HdWallet (
    -- * Supporting types
    WalletName(..)
  , AccountName(..)
  , HdAccountIx(..)
...
import           Seal.Crypto (HDPassphrase)
...
import           Seal.Wallet.Kernel.DB.Util.IxSet hiding (foldl')
import qualified Seal.Wallet.Kernel.DB.Util.IxSet as IxSet hiding (Indexable)
import qualified Seal.Wallet.Kernel.DB.Util.Zoomable as Z
import           Seal.Wallet.Kernel.Ed25519Bip44 (ChangeChain (..),
...
import           Seal.Wallet.Util (buildTrunc)
...
data UnknownHdAddress =
  ...
  | UnknownHdSealAddress Core.Address
...
instance Arbitrary UnknownHdAddress where
    arbitrary = oneof [ UnknownHdAddressRoot <$> arbitrary
                      ...
                      , UnknownHdSealAddress <$> arbitrary
                      ]
...
-- | Zoom to the specified Seal address
--
...
zoomHdCardanoAddress embedErr addr =
  ...
  where
    ...
    err = missing $ embedErr (UnknownHdSealAddress addr)
...
instance Buildable UnknownHdAddress where
    ...
    build (UnknownHdSealAddress coreAddress) =
        bprint ("UnknownHdSealAddress " % build) coreAddress
