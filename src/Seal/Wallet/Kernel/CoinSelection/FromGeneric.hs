...
module Seal.Wallet.Kernel.CoinSelection.FromGeneric (
...
import           Seal.Binary.Class (LengthOf, Range (..), SizeOverride (..),
                     encode, szSimplify, szWithCtx, toLazyByteString)
import           Seal.Chain.Txp (TxAux, TxIn, TxInWitness, TxOut, TxSigData)
import           Seal.Chain.Txp as Core (TxIn, TxOutAux, Utxo, toaOut,
                     txOutAddress, txOutSeal)
import           Seal.Core as Core (AddrAttributes, Address, Coin (..),
                     TxSizeLinear, calculateTxSizeLinear,
                     isRedeemAddress, maxCoinVal)

import           Seal.Core.Attributes (Attributes)
import           Seal.Crypto (Signature)
import           Serokell.Data.Memory.Units (Byte, toBytes)

import           Seal.Wallet.Kernel.CoinSelection.Generic
import           Seal.Wallet.Kernel.CoinSelection.Generic.Fees
import           Seal.Wallet.Kernel.CoinSelection.Generic.Grouped
import qualified Seal.Wallet.Kernel.CoinSelection.Generic.LargestFirst as LargestFirst
import qualified Seal.Wallet.Kernel.CoinSelection.Generic.Random as Random
...
instance IsValue Core.Coin where
  ...
  valueAdd    = addCoin
  valueSub    = subCoin
  valueDist   = \  a b -> if a < b then b `unsafeSubCoin` a
                                   else a `unsafeSubCoin` b
...
instance CoinSelDom Cardano where
  ...
  outVal    = Core.txOutSeal   . Core.toaOut
  outSubFee = \(Fee v) o -> outSetVal o <$> valueSub (outVal o) v
     where
       outSetVal o v = o {Core.toaOut = (Core.toaOut o) {Core.txOutSeal = v}}
...
runCoinSelT opts pickUtxo policy (List.sortBy (flip (comparing outVal)) -> request) =
  ...
  where
    ...
    pickUtxo val = search . Map.toList =<< get
      where
        ...
        search ((i, o):ios)
          | Core.txOutSeal (Core.toaOut o) >= val = return $ Just (i, o)
...
addCoin :: Core.Coin -> Core.Coin -> Maybe Core.Coin
addCoin (unsafeGetCoin -> a) (unsafeGetCoin -> b)
    | res >= a && res >= b && res <= unsafeGetCoin (maxBound @Core.Coin) = Just (Core.Coin res)
    | otherwise = Nothing
  where
    res = a+b                          
...
divCoin :: Integral b => Core.Coin -> b -> Core.Coin
divCoin (unsafeGetCoin -> a) b = Core.Coin (a `div` fromIntegral b)
...
subCoin :: Core.Coin -> Core.Coin -> Maybe Core.Coin
subCoin (unsafeGetCoin -> a) (unsafeGetCoin -> b)
    | a >= b = Just (Core.Coin (a-b))
    | otherwise = Nothing

unsafeSubCoin :: Core.Coin -> Core.Coin -> Core.Coin
